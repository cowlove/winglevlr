...
test.out:	winglevlr_ubuntu
	./winglevlr_ubuntu  --serial --seconds 700  | uniq | tee $@
...
simplot:	test.out
	cat test.out | grep -a srv > /tmp/simplot.txt && gnuplot -e 'f= "/tmp/simplot.txt"; p f u 1:5 w l t "Pitch", f u 1:3 w l t "Roll"; pause 111'
